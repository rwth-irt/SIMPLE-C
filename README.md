...
1.  Run the ROS2 Docker container with access to source code and rosbag data:
    ```sh
    sudo docker run --volume <THIS_REPO>:/calib_src --volume <DATA_DIRECTORY>:/DATA:ro -it <VOLUME_TAG> bash
    ```
    Note that we need python dependencies such as `sklearn` in there. For this you can use the (temporary) Dockerfile to build
    a working image: `sudo docker build -t <VOLUME_TAG> .`
